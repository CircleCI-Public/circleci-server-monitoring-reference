...
| tempo.storage | object | `{"traces":{"backend":"memory","size":"50Gi"}}` | Storage configuration for trace data |
...
| tempo.storage.traces.size | string | `"50Gi"` | Storage volume size For memory/pv: actual volume size For cloud backends: size of WAL (Write-Ahead Log) volume Increase for higher trace volumes or longer retention |
